...
class PrevisionAdmin(admin.ModelAdmin):
    ...
    def full_url(self, obj: Prevision):
        return f'https://www.{hostname}{reverse("prevision-detail", args=[obj.id, slugify(obj.name)])}'
...
class FuturoAdmin(admin.ModelAdmin):
    ...
    def full_url(self, obj: Futuro):
        return f'https://www.{hostname}{reverse("futuro-detail", args=[obj.id, slugify(obj.name)])}'
